...
' _ "$ROOT" "$dir/home/state" "$dir/home/state/different-task.status" \
  || fail "old announcement setup did not mark the status current"
...
[ "$first_rc" -ne 0 ] || fail "the superseded concurrent arm reported success"
...
pass "root publication locking leaves the last concurrent arm valid"
...
grep -qxF 'pr=https://github.com/acme/widget/pull/99' \
  "$dir/home/state/different-task.meta" \
  || fail "failed root poll publication did not retain its repairable PR metadata"
...
FM_ROOT_OVERRIDE="$ROOT" \
  ...
  || fail "an exact retry did not repair a failed root publication"
FM_ROOT_OVERRIDE="$ROOT" bash -c '
  . "$1/bin/fm-pr-lib.sh"
  fm_pr_poll_artifacts_valid "$2" different-task "$1/bin/fm-pr-poll.sh"
' _ "$ROOT" "$dir/home/state" \
  || fail "the repaired root publication did not leave a valid poll"
...
sed '/^pr=/d; /^pr_head=/d' "$dir/home/state/different-task.meta" > "$dir/meta.clean"
mv "$dir/meta.clean" "$dir/home/state/different-task.meta"
publication_rc=0
FM_ROOT_OVERRIDE="$ROOT" \
  FM_HOME="$dir/home" \
  FM_STATE_OVERRIDE="$dir/home/state" \
