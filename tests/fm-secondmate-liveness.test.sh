...
test_tmux_agent_state_classifies() {
  ...
  for harness in claude codex opencode omp grok kimi pi pi-signed pi-launcher Pi; do
...
case "${1:-}" in
  ...
  list-windows)
    case "$mode" in
      missing)
        printf '@0 main\n'
        [ ! -e "${FM_TMUX_CALL_LOG:?}.created" ] || printf '@1 fm-sm1\n'
        exit 0
        ;;
      unreadable) exit 1 ;;
      *) [ -e "${FM_TMUX_CALL_LOG:?}.killed" ] || printf '@1 fm-sm1\n'; exit 0 ;;
    esac
    ;;
