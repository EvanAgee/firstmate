#!/usr/bin/env bash
# Focused OMP capability-probe contract tests.
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

TMP_ROOT=$(fm_test_tmproot fm-omp-capabilities)
...
test_capability_probe_reports_every_missing_requirement() {
  ...
  for capability in '--model=' '--thinking=' '--auto-approve' '--session-dir=' '--extension=' '--resume=' '--config=' '--cwd='; do
