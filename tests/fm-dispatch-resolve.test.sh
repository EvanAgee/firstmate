...
# codex members, so without a stub on PATH every case below would reach the
# REAL binary and depend on the captain's live subscription balances: a model
# the account has genuinely spent drops its rung and the pool under test
# shrinks. (Measured 2026-09-22: model:fable at 0% remaining, which broke the
# list-order tie case, whose pool names claude/fable.) This default reports
# every provider healthy at the account scope and names no model scope, so
# model_exhausted finds no model window and excludes nothing. The cases that
# are ABOUT quota put their own quota-axi earlier on PATH and still win.
DEFAULT_QUOTA_FAKEBIN=$(fm_fakebin "$TMP_ROOT/default-quota")
cat > "$DEFAULT_QUOTA_FAKEBIN/quota-axi" <<'SH'
#!/usr/bin/env bash
set -u
provider=claude
case "$*" in
...
JSON
SH
chmod +x "$DEFAULT_QUOTA_FAKEBIN/quota-axi"
