...
#!/usr/bin/env bash
set -u
# The root's fm-crew-state.sh selects the run from the bare `axi` overview
# table before it trusts `axi status`, so the fake serves both.
if [ "${1:-}" = axi ] && [ "$#" -eq 1 ]; then
...
runs[1]{id,branch,status,head,pr}:
  "fixture-run",$branch,running,$head,""
EOF
  exit 0
fi
