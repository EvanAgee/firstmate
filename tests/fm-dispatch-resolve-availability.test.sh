...
# The resolver's own model_exhausted filter reads quota-axi for claude and
# codex members, and only the clean-probe case below substituted a stub, so
# every other case reached the REAL binary. No case depends on that today
# (the claude/fable rung here is already quarantined, so it is excluded
# before quota is consulted, and claude/opus and codex/gpt-5 carry no
# model-scoped window), but that is a property of the captain's current
# account rather than of these tests: the sibling
# tests/fm-dispatch-resolve.test.sh broke exactly this way when model:fable
# reached 0% remaining. Installing the same healthy probe file-wide keeps the
# pause and quarantine verdicts the only variable regardless of live balances;
# the cases that substitute their own quota-axi put it earlier on PATH and win.
DEFAULT_QUOTA_FAKEBIN=$(fm_fakebin "$TMP_ROOT/default-quota")
...
write_clean_quota_axi "$DEFAULT_QUOTA_FAKEBIN"
PATH="$DEFAULT_QUOTA_FAKEBIN:$PATH"
export PATH
