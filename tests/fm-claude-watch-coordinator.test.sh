...
# The production owner of "this file's reported state was already surfaced"
# (bin/fm-wake-lib.sh's fm_wake_status_mark_current, via tests/wake-helpers.sh)
# is the only thing that can prime it, so no test hand-writes the marker shape.
prime_seen() {  # <dir> <status-file>
  prime_status_seen "$1/state" "$2"
}
