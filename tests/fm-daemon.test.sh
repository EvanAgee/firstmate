...
test_reported_stall_identity_survives_housekeeping() {
  ...
  (
    ...
    [ "$(cat "$state/.subsuper-stalled-stalled.generation")" = 0 ] || fail "known recovery changed the delivered generation"
...
test_inject_msg_herdr_submits_through_backend_dispatch() {
  ...
  (
    ...
    fm_backend_send_text_submit() {
      ...
      [ "${10}" = /verified/bin ] || fail "OMP Herdr injection did not forward its bound OMP entry identity: ${10:-missing}"
      printf 'empty'
    }
    FM_SUPERVISOR_BACKEND=herdr FM_SUPERVISOR_TARGET="default:w1:p2" \
      FM_SUPERVISOR_HARNESS=omp FM_SUPERVISOR_OMP_BUN=/verified/bun FM_SUPERVISOR_OMP_BIN=/verified/bin \
