...
# The sweep's registry check compares a canonical slot path against the raw
# path recorded in treehouse-state.json. macOS TMPDIR sits under the /var ->
# /private/var symlink, so canonicalize the fixture root once here; the real
# pool root has no symlinked ancestor.
...
# --- a held project lock refuses the apply pass -------------------------------

lock_held_marker="$TMP_ROOT/lock-held"
# shellcheck disable=SC2016 # The inner shell expands its own positional parameters.
env "${SWEEP_ENV[@]}" bash -c '
  . "$1/bin/fm-homes-lib.sh"
  lock=$(fm_treehouse_project_lock_path "$2") || exit 1
  fm_lock_try_acquire "$lock" || exit 1
  : > "$3"
  exec sleep 30
' _ "$ROOT" "$REPO" "$lock_held_marker" &
lock_holder_pid=$!
for _ in $(seq 1 100); do
  [ -f "$lock_held_marker" ] && break
  sleep 0.05
done
[ -f "$lock_held_marker" ] || fail "the fixture lock holder never acquired the project lock"
if out=$(run_sweep --pool "$REPO" --apply-slot "$(slot 2)" --captain-approved 2>&1); then
  kill "$lock_holder_pid" 2>/dev/null
  fail "--apply-slot ran while the project lock was held"
fi
assert_contains "$out" "holds the project lock" "the held-lock refusal is missing"
kill "$lock_holder_pid" 2>/dev/null
wait "$lock_holder_pid" 2>/dev/null
pass "a held project lock refuses the apply pass"

# --- unprovable occupancy refuses the apply pass ------------------------------

# Force both occupancy probes to fail: the /proc scan on Linux and the lsof
# fallback elsewhere.
if out=$(env "${SWEEP_ENV[@]}" FM_PROC_ROOT_OVERRIDE="$TMP_ROOT/no-proc" FM_POOL_LSOF_CWD_FILE="$TMP_ROOT/no-lsof" \
  "$ROOT/bin/fm-treehouse-sweep.sh" --pool "$REPO" --apply-clean 2>&1); then
  fail "--apply-clean ran while occupancy was unprovable"
fi
assert_contains "$out" "occupancy is unprovable" "the unprovable-occupancy refusal is missing"
pass "unprovable occupancy refuses the apply pass"
