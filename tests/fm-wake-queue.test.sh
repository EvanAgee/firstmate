...
test_stale_enqueue_before_suppressor() {
  ...
  sig=$(bash -c '. "$1/bin/fm-wake-lib.sh"; fm_wake_signal_sig "$2"' _ "$ROOT" "$state/stale.status")
...
test_not_working_stale_enqueue_before_suppressor() {
  ...
  sig=$(bash -c '. "$1/bin/fm-wake-lib.sh"; fm_wake_signal_sig "$2"' _ "$ROOT" "$state/stopped.status")
...
test_interruption_before_and_after_raw_commit() {
  ...
  before_ready="$state/.wake-drain-test-before-commit-ready"
  ...
  pid=$!
  i=0
  while [ "$i" -lt 200 ] && [ ! -e "$before_ready" ]; do
    sleep 0.05
    i=$((i + 1))
  done
  [ -e "$before_ready" ] || { kill "$pid" 2>/dev/null || true; fail "pre-commit drain never reached its interruption point"; }
