...
test_stale_enqueue_before_suppressor() {
  ...
  if [ "$(uname)" = Darwin ]; then sig=$(stat -f '%z:%i:%Fm' "$state/stale.status"); else sig=$(stat -c '%s:%i:%.9Y' "$state/stale.status"); fi
...
test_not_working_stale_enqueue_before_suppressor() {
  ...
  if [ "$(uname)" = Darwin ]; then sig=$(stat -f '%z:%i:%Fm' "$state/stopped.status"); else sig=$(stat -c '%s:%i:%.9Y' "$state/stopped.status"); fi
