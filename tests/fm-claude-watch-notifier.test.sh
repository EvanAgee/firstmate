...
test_parent_term_records_interrupted() {
  ...
  # The owner lock is the claim ledger's micro-mutex here, not the notifier's
  # park ownership: the notifier holds no mutex across its park (a parked hook can
  # block for hours), so the ported design asserts the invariant that matters
  # instead - no LIVE holder may survive the kill. A second group TERM can land
  # while the interrupted record is being written, and a process killed inside a
  # lock's critical section always leaves a reclaimable record behind
  # (bin/fm-wake-lib.sh's steal path owns that recovery), so a residual lock whose
  # recorded pid is dead is litter, never a wedge.
  sleep 1
  lock_pid=$(cat "$dir/state/.claude-autoarm.lock/pid" 2>/dev/null || true)
  case "$lock_pid" in
    ''|*[!0-9]*) : ;;
    *) kill -0 "$lock_pid" 2>/dev/null && fail "parent TERM left a LIVE owner lock (pid $lock_pid)" ;;
  esac
  [ "$(epoch_outcome "$dir")" = interrupted ] || fail "an owner-lock write overwrote the interrupted outcome"
  pass "notifier: parent TERM records interrupted and leaves no live owner lock"
}
