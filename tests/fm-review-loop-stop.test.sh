...
test_root_classifier_reads_stop_decision() {
  local task=classifier-integration run=review-run home head rc open
  home=$(make_home classifier-integration "$task")
  for head in first second third; do
    if record "$home" "$task" "$run" "$head" "Tried $head fix." \
      defect:repeat --targeted defect:repeat >/dev/null 2>&1; then
      rc=0
    else
      rc=$?
    fi
  done
  expect_code 20 "$rc" "third repeated review should stop"
  open=$(bash -c '. "$1"; status_open_decisions "$2"' _ \
    "$ROOT/bin/fm-classify-lib.sh" "$home/state/$task.status")
  assert_contains "$open" $'review-loop-review-run-1\tneeds-decision\t' \
    "root status classifier did not retain the review stop as an open decision"
  pass "review-loop stop: root classifier sees the open decision"
}
