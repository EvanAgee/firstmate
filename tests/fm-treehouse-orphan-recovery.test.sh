...
# This port carries the upstream Treehouse sweep, not the upstream acquisition
# scripts this test exercises; the fork acquires slots by running the
# `treehouse` CLI directly. Skip when those scripts are absent rather than
# failing on a missing subject.
for required in fm-treehouse-get.sh fm-treehouse-command.sh treehouse-git-guard/git; do
  [ -e "$ROOT/bin/$required" ] || { echo "skip: bin/$required is not present in this fork"; exit 0; }
done
