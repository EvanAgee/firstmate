...
run_case \
  ...
  "SELECTED=claude" \
  "claude=0,codex=3" \
  "Resolve this matched dispatch pool now. Load quota-array-dispatch and run quota-axi --json exactly once. Both pool members have comparable required task fit and the same strongest reasoning class. The authoritative catalogs already prove Claude/Sonnet and Codex/GPT models supported in their stated provider families, and their selected authentication surfaces are usable. The likely task-completion horizon is two hours with established confidence. Return exact lines FACT=claude|live=0|headroom=1|runway_seconds=10800|reserve=-1 and FACT=codex|live=3|headroom=55|runway_seconds=14400|reserve=-40 to preserve candidate accounting, then an exact final line SELECTED=<claude|codex>. Do not use other vendor or model commands and do not modify files." \
...
run_case \
  ...
  "SELECTED=codex" \
  "claude=0,codex=0" \
  "Resolve this matched dispatch pool now. Load quota-array-dispatch and run quota-axi --json exactly once. Both pool members have comparable required task fit and the same strongest reasoning class. The authoritative catalogs already prove Claude/Sonnet and Codex/GPT models supported in their stated provider families, and their selected authentication surfaces are usable. The likely task-completion horizon is two hours with established confidence. Return exact lines FACT=claude|live=0|headroom=1|runway_seconds=10800|reserve=-1 and FACT=codex|live=0|headroom=55|runway_seconds=14400|reserve=-40 to preserve candidate accounting, then an exact final line SELECTED=<claude|codex>. Do not use other vendor or model commands and do not modify files." \
  "FACT=claude|live=0|headroom=1|runway_seconds=10800|reserve=-1" \
...
run_case \
  ...
  "DECISION=CLAUDE" \
  "claude=0,codex=0" \
  "Resolve this matched dispatch pool now. Load quota-array-dispatch and run quota-axi --json exactly once. Both pool members have comparable required task fit and the same strongest reasoning class. The authoritative catalogs already prove both models supported in their stated provider families, and their selected authentication surfaces are usable. The likely task-completion horizon is two hours with established confidence. Claude has higher known headroom but explicitly unmeasurable runway, while Codex has lower known headroom and established runway that supports completion. Unmeasurable runway does not drop a member; both stay eligible and the uncertainty must be disclosed. Return exact lines FACT=claude|eligible=yes|live=0|headroom=55|runway=unknown|unmeasurable=weekly and FACT=codex|eligible=yes|live=0|headroom=45|runway_seconds=14400|supports_horizon=yes, then an exact final line DECISION=<CLAUDE|CODEX>. Do not use other vendor or model commands and do not modify files." \
