...
| Skill              | What it does                                                                                                                                  |
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `/afk`             | Enter away-mode supervision: the sub-supervisor self-handles routine notifications in bash, escalates diagnosed stalled runs, captain-relevant events, and bounded declared-external-wait rechecks as batched digests, and actively alerts if delivery gets stuck while you step away |
...
| `/bakeoff`         | Compare harness and model combinations on one identical coding task in isolated worktrees, score the results, and publish a Lavish comparison |
...
- `.agents/skills/` - agent-loaded skills (this section's table, firstmate's agent-only reference skills, and a vendored third-party set).
  Firstmate-owned skills in this tree assume a live firstmate home and are meaningless, or actively misleading, installed anywhere else, so each carries `metadata.internal: true` in its frontmatter.
  ...
  The same tree also holds a vendored set from `mattpocock/skills`, copied verbatim and recorded in [`skills-lock.json`](skills-lock.json) by upstream repo, path, and content hash.
  Those files keep their upstream frontmatter and therefore do not carry `metadata.internal: true`.
  Do not edit them, because a local edit would break the lock file's hash.
...
- [CONTEXT.md](CONTEXT.md) - project glossary for captain, crewmate, task, and the other words the API uses.
...
- [docs/wedge-alarm.md](docs/wedge-alarm.md) - configure the active alert for a stuck away-mode escalation and the session-independent watcher-down alert.
