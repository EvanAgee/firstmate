...
. "/Users/evanagee/.no-mistakes/worktrees/b99440365b40/01M2GF8XHV1VDP27TYFRYWYRJA/tests/lib.sh"
...
run_watcher_bounded() {
  ...
  perl -e 'my $pid=fork; die unless defined $pid; if (!$pid) { exec @ARGV } local $SIG{ALRM}=sub { kill "TERM", $pid; waitpid $pid, 0; exit 124 }; alarm 90; waitpid $pid, 0; alarm 0; exit($? >> 8)' \
    env FM_HOME="$home" FM_ROOT_OVERRIDE="$watch_root" FM_GH_HEALTH_PROBE_CMD=: \
      FM_CHECK_INTERVAL="$check_interval" FM_CHECK_TIMEOUT="${FM_TEST_CLOSE_BOUND:-15}" \
...
test_merged_poll_closes_linked_issues() {
  ...
  rc=$?
  set -e
  [ "$rc" -eq 0 ] || fail "merged issue-close watcher failed (rc=$rc): $(cat "$dir/watch.err")"
...
test_merged_poll_issue_close_failure_keeps_the_merged_wake() {
  ...
  grep -qF 'linked issues were not all closed for task-a after https://github.com/o/r/pull/1 merged' \
    "$state/.watch-triage.log" \
    || fail "a failed issue close left no record in the triage log: $(cat "$state/.watch-triage.log" 2>/dev/null)"
  assert_poll_absent "$state" task-a
...
test_merged_poll_issue_close_timeout_keeps_the_merged_wake() {
  ...
  set +e
  FM_TEST_CLOSE_BOUND=1 FM_CHECK_FORCE_FALLBACK=1 FM_TEST_GH_STATE=MERGED FM_TEST_GH_AXI_PR_STATE=merged \
...
test_merged_poll_logs_an_already_closed_receipt() {
  ...
  set +e
  FM_TEST_GH_STATE=MERGED FM_TEST_GH_AXI_PR_STATE=merged FM_TEST_ISSUE_STATE=closed \
  FM_TEST_GH_AXI_LOG="$dir/gh-axi.log" \
    run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/watch.out" 2> "$dir/watch.err"
  rc=$?
  set -e
  ...
  assert_no_grep 'issue close' "$dir/gh-axi.log" \
    "an issue that was already closed was closed again"
  grep -qF 'task-a issue close after https://github.com/o/r/pull/1 merged: already-closed: o/r#55' \
    "$state/.watch-triage.log" \
    || fail "an already-closed issue left no receipt in the triage log: $(cat "$state/.watch-triage.log" 2>/dev/null)"
  ack_watcher_cycle "$state" || fail "already-closed issue acknowledgement failed"
  pass "a merged poll records an already-closed linked issue without mutating it"
}
...
fail() {
  printf 'not ok - %s\n' "$1" >&2
  cp -R "$TMP_ROOT" "$FM_PHASE_EVIDENCE/watch-budgeted-failed-fixtures"
  exit 1
}
...
python3 - "$TMP_ROOT" "$FM_PHASE_EVIDENCE" "watch" <<'PYCOLLECT'
from pathlib import Path
import sys
src,ev,label=Path(sys.argv[1]),Path(sys.argv[2]),sys.argv[3]
names={'out','err','stdout','stderr','watch.out','watch.err','gh-axi.log','gh.log','.watch-triage.log','.wake-queue'}
with (ev/(label+'-transcript.txt')).open('w') as out:
    out.write('Executable CLI integration tests using isolated task metadata and fake forge commands. No live issue mutations.\n')
    for case in sorted(src.iterdir()):
        if not case.is_dir(): continue
        out.write('\nCASE '+case.name+'\n')
        for p in sorted(case.rglob('*')):
            if p.is_file() and p.name in names:
                out.write('--- '+str(p.relative_to(case))+' ---\n'+(p.read_text() or '(empty)\n'))
        state=case/'home/state'
        if state.is_dir():
            present=[p.name for p in state.glob('task-a.*') if p.suffix in ('.sh','.pr-poll','.pr-poll-registration','.pr-poll-retirement')]
            out.write('Remaining task-a poll artifacts: '+str(present)+'\n')
PYCOLLECT
