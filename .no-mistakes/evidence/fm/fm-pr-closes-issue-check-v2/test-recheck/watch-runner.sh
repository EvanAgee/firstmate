...
run_watcher_bounded() {
  ...
  perl -e 'my $pid=fork; die unless defined $pid; if (!$pid) { exec @ARGV } local $SIG{ALRM}=sub { kill "TERM", $pid; waitpid $pid, 0; exit 124 }; alarm 90; waitpid $pid, 0; alarm 0; exit($? >> 8)' \
    env FM_HOME="$home" FM_ROOT_OVERRIDE="$watch_root" FM_GH_HEALTH_PROBE_CMD=: \
      FM_CHECK_INTERVAL="$check_interval" FM_CHECK_TIMEOUT="${FM_PHASE_CHECK_TIMEOUT:-15}" \
...
test_merged_poll_issue_close_timeout_keeps_the_merged_wake() {
  ...
  set +e
  FM_PHASE_CHECK_TIMEOUT=1 FM_CHECK_FORCE_FALLBACK=1 FM_TEST_GH_STATE=MERGED FM_TEST_GH_AXI_PR_STATE=merged \
