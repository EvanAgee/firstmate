...
A rung marked `"enabled": false` is switched off by the captain: drop it before selection, never re-enable it on quota evidence, and stop and ask when that empties a ladder ([`docs/configuration.md`](docs/configuration.md) "Crew dispatch profiles").
