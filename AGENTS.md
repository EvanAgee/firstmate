...
state/               runtime records and signals; gitignored
  ...
  captain-replies.jsonl captain-replies.cursor .captain-queue.lock  dashboard answers to fleet-board cards, reconcile cursor, and add/reconcile lock; bin/fm-captain-queue.sh
...
Each unpinned rule's `use` array is a pool: every member holds that category's quality floor and is good enough for the work, so new tasks spread evenly across the pool rather than piling onto whichever member comes first.
Pick an unpinned pool member by round-robin: choose the enabled, healthy member currently carrying the fewest live workers from this pool in this home, break ties by most quota headroom, then by list order.
In an unpinned pool, drop a member marked `"enabled": false` before counting live workers or selecting, never re-enable it on any evidence, and stop and ask when that empties the pool ([`docs/configuration.md`](docs/configuration.md) "Crew dispatch profiles").
Quota's role in an unpinned pool is a health filter plus tie-break, not the selector: drop a member that is unhealthy or genuinely quota-tight, then round-robin among the rest for even spread rather than maximizing quota.
...
Missing model-level quota, a missing authentication source, unmeasurable headroom, or unmodeled authentication is disclosed uncertainty that keeps a member eligible, never a credential or login escalation.
In an unpinned pool, only concrete contradictory evidence drops a member on health, such as an authoritative catalog proving the model unsupported or proof that the credential selected for that surface is unusable; never infer a credential store, provider family, or quota mapping from a harness, model, or source name, and never launch another harness's CLI to judge a member.
Preserve malformed profile configuration as an actionable error rather than selecting around it.
When a whole unpinned pool is quota-tight, preserve the captain's strongest-reasoning class rather than silently downgrading it solely to conserve quota; stop and report the tight choice if that class cannot proceed.
A worker already running finishes on the runtime it launched with; round-robin only sets where new work lands in an unpinned pool.
`quota-axi` owns how model or product windows relate to bounding account windows and remains data-only.
Load `quota-array-dispatch` before choosing among a pool with more than one member; that skill is the single owner of the round-robin selection procedure.
...
If a ship task explicitly adopts the Matt flow, scaffold with `--matt-flow`; it emits one thin trigger and the installed skills own every phase, so never copy flow content into an ordinary brief.
...
- `quota-array-dispatch` - load before choosing among a crew-dispatch pool with more than one member.
