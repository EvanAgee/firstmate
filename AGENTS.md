...
config/wedge-alarm  optional away-mode wedge-alarm and watcher-beat-alert active-alert directives; LOCAL, gitignored; absent means auto (macOS Notification Center when available); see docs/wedge-alarm.md
...
state/               runtime records and signals; gitignored
  ...
  .claude-autoarm.lock .claude-autoarm-epoch .claude-autoarm-failure-notified .claude-autoarm-failure-alarmed .claude-coordinator.lock .claude-coordinator-generation .claude-ready-to-notify .claude-notifier-surfaced-seq .turnend-claude-blocks .turnend-claude-blocks.lock   Claude Stop coordinator/notifier single-flight, epoch, failure-episode, ready-to-notify, attended-alarm, guard-budget, and budget-lock records; never touch
  .session-odometer .last-anchor   session-age and handled-wake counters plus the last printed heartbeat ANCHOR mtime; written only by bin/fm-anchor-lib.sh via bin/fm-wake-drain.sh; safe to delete
