...
omp uses the tracked `.pi/extensions/fm-primary-omp-turnend-guard.ts` plus `.pi/extensions/fm-primary-omp-watch.ts`, both loaded explicitly with `-e` because omp does not auto-discover project extensions.
