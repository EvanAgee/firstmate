...
| Fact | Value |
|---|---|
| Binary | `omp`, a single Bun-compiled executable resolved from `PATH` and capability-checked by `../../../bin/fm-omp-capabilities.sh` (Bun-backed entrypoint plus the model, thinking, unattended approval, external extension, session-directory, resume, config, and cwd flags); a missing binary or an incomplete capability surface refuses the spawn, and selected omp never falls back to another harness. |
