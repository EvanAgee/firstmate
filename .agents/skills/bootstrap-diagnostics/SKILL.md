...
- `MISSING: <tool> (install: <command>)` - list the missing tools to the captain with a one-line purpose each plus the printed install commands, wait for consent (one approval may cover the list), then run `bin/fm-bootstrap.sh install <approved tools...>`.
  ...
  For any required axi-family tool - `gh-axi`, `chrome-devtools-axi`, `lavish-axi`, or `tasks-axi` - an installed version below its floor is a plain upgrade request; [`bin/fm-bootstrap.sh`](../../../bin/fm-bootstrap.sh) owns the floor policy, and never argue the floor down to whatever the home happens to have installed.
  ...
  For `chrome-devtools-axi`, this additionally covers an installed build that fails the named-session open-and-snapshot probe (`bin/fm-chrome-devtools-axi-lib.sh` owns the definition), including the `pageId` schema break from an unpinned `chrome-devtools-mcp@latest`.
...
- `TANGLE: <remediation>` - the primary checkout is stranded on a feature branch instead of its default branch; `AGENTS.md` section 8 says to resolve it without touching unlanded work, sections 7 and 11 require project work to start in an isolated worktree, and the `supervision-reference` skill records why the guard exists.
...
- `CREW_DISPATCH: invalid config/crew-dispatch.json - <reason>` - the optional dispatch profile file exists but failed low-cost bootstrap validation; stop profile-based dispatch, report the printed reason, and require correction rather than falling back around it or selecting a bad profile.
  The valid schema and invalid-config cases are owned by [`docs/configuration.md`](../../../docs/configuration.md) "Crew dispatch profiles".
...
- `PR_CHECK_MIGRATION: canonical polls rebuilt and armed (recovers watches left dormant by a prior release or a macOS reboot); resume supervision for this home` - the non-executing migration rebuilt canonical task polls from validated metadata, and those polls are already armed.
  A macOS reboot reassigns the state directory's device number, so a poll registration written before the inode-only format stops parsing and is rebuilt here instead of staying armed but silently dormant.
