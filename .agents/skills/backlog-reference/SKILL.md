...
description: >-
  ...
  Use before filing, holding, parking, closing, or handing off a backlog item, and before rewriting a backlog item's notes.
...
`AGENTS.md` section 10 keeps the work-items-only contract and update cadence inline; this skill holds the procedures it summarizes.
...
When the captain parks a captain-gated thread for later, use `bin/fm-decision-hold.sh park`; `decision-hold-lifecycle` owns the policy.
