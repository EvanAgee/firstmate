...
Answer-time hold closure verified on 2026-08-16 on the same platform, against the same published poll response shape.
...
| adapter-owned answer-time hold closure | `tests/fm-decision-hold-lifecycle.test.sh` proves that a Lavish review armed with `--decisions-origin` closes the captain holds its captured structured answers name, at capture time; that a key appearing only in freeform captain prose closes nothing; that a hold still blocking routed work is skipped rather than forced and stays available to `resolve`; that a replayed capture is idempotent; that a deck armed with no decision origin closes nothing at all; and that the runner's `autohandle` entry applies those closures while still leaving the capture unacknowledged, so its `check` wake still reaches the handler |
