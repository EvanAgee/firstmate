...
`bin/fm-guard.sh`, the pull warning, instead uses the model-aware `fm_watcher_supervision_verdict` from the same library, because it fires mid-turn.
Claude's coordinator (`bin/fm-claude-watch-coordinator.sh`) now keeps a live watcher across the handling turn, so the ordinary mid-turn state is a live identity-matched process with a fresh beacon, while Cursor still parks between turns and Claude keeps the `autoarm` pull-guard classification.
Under the inherited Claude `autoarm` pull-guard model a beacon fresh within grace is healthy even with no live watcher process.
...
A fixed 300-second grace default stops correctly bounding staleness once a home's `FM_POLL` reaches or exceeds it: a perfectly healthy watcher mid-wait would then read stale at the edge of every full poll cycle by definition, which is exactly what a long-poll home (`FM_POLL=300`) hit against the Claude Stop-hook between-turns arm.
...
- Cursor registers a `stop` hook in `.cursor/hooks.json` and delegates the whole turn boundary to `bin/fm-turnend-guard-cursor.sh`, the park described below.
  ...
  Cursor 2026.08.11-e8db854 does not fire the Claude-shaped `Stop` entries at all, but they are guarded anyway because Cursor has neither `async` nor `asyncRewake`: if a later build did fire them, `bin/fm-claude-watch-coordinator.sh` or `bin/fm-claude-watch-notifier.sh` would run synchronously inside Cursor's stop step and hold that turn open for its declared multi-hour timeout, exactly the wedge grok 1.0.0 produced.
- Grok registers a `Stop` hook in `.grok/hooks/fm-primary-turnend-guard.json` and delegates capability selection to `bin/fm-turnend-guard-grok.sh`.
  ...
  A guard keyed on `GROK_AGENT` alone therefore stopped firing on grok 1.0.0, and the resulting Claude-only Stop hook ran synchronously under Grok - Grok has neither `async` nor `asyncRewake`, so it waited on the watcher for the declared 28800-second timeout and the Grok turn never ended.
...
Claude runs the guard with `--claude`, which ignores `stop_hook_active` and cooperates with the Stop-owned notifier.
...
The Claude mode waits up to `FM_CLAUDE_AUTOARM_SYNC_WAIT_MS` (default 800 milliseconds) and allows the stop when the watcher is healthy, the notifier's generation claim is open, or `state/.claude-autoarm-epoch` contains a fresh actionable rewake owned by this event epoch.
A `coordinator` lock alone never satisfies this guard: `bin/fm-claude-watch-coordinator.sh` cannot exit 2, so its state is diagnostic evidence, never notification ownership.
...
A claim is open while its outcome is `arming` or `parked`, its owner pid is alive, its recorded identity successfully recomputes and matches that pid, and it is not stuck - stuck meaning the entry and the watcher beacon are both older than the guard grace, which proves the owner hung with nothing supervising (a healthy hours-long cycle keeps the beacon beating, and every arming phase with no watcher is bounded in seconds).
`parked` is the Claude notifier's long wait between turns: it owns the exit-2 wake path for its event epoch and holds no mutex while it waits, and its own bounded coordinator-absent wait keeps a parked claim from outliving the grace window with nothing supervising.
...
No mutex is held across a blocking step: `state/.claude-autoarm.lock` survives only as a micro-mutex serializing individual ledger writes, and the parked notifier holds none while it waits, and a superseded owner goes completely silent - ownership is re-verified before every arm invocation, episode-state mutation, ledger write, and continuation.
...
The one loud attended fail-open is available only when the notifier has recorded an exhausted failure, its one notice is already consumed, the block budget is exhausted, and a final check finds neither a healthy watcher nor an automatic continuation.
...
Whenever both coordination locks are needed, positive notifier recovery and the terminal check acquire the notifier owner lock before the budget lock.
After that alarm, the notifier suppresses further exit-2 continuations until positive watcher recovery, so the final fail-open remains reachable.
...
This remains a between-turns park, so `fm_supervision_model` classifies Cursor as `autoarm` and the mid-turn pull guard accepts a fresh beacon without a live watcher.
