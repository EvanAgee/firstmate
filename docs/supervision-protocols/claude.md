...
8. The turn-end guard (`bin/fm-turnend-guard.sh --claude`) remains the final backstop.
   ...
   Otherwise, it allows the stop when a watcher is healthy or an open notifier claim owns recovery, while fresh failure epochs advance the bounded one-time attended fail-open progression described there.
