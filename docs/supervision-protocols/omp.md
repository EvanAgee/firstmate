...
2. Confirm the omp primary loaded both extensions.
   omp does NOT auto-discover project `.pi/extensions`.
   If the running session has not loaded both, relaunch with `bin/fm-omp.sh`, or restart omp with `-e __FM_OMP_TURNEND_EXT__ -e __FM_OMP_EXT__`.
...
10. Missing, failed, or unhealthy cycle only: if a later notification explicitly reports one of those repair conditions, drain queued wakes, inspect the failure text, call `fm_watch_arm_omp`, and relaunch with `bin/fm-omp.sh` if needed.
