...
- A route with no probe source at all, and the Gateway while its `reports[]` is empty, are distinct cases from the above and are never `unknown`; see the no-probe rule under "Evidence sources".
