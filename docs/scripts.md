...
| `fm-api.sh`              | Start, stop, and inspect this home's localhost API server |
...
| `fm-captain-queue.sh`    | Write fleet-board captain cards, reconcile answers, defer confirmed-unbacked cards at any age, migrate verified legacy cards, expire remaining unbacked cards into parked, and retire backed cards whose work is done |
...
| `fm-session-lock-lib.sh` | Shared session-lock harness identity (ancestry walk and holder liveness) for fm-lock.sh and the Claude Stop coordinator/notifier |
...
| `fm-wedge-alarm-lib.sh`  | Shared active-alert channel machinery for the away-mode wedge alarm and watcher-beat alert |
...
| `fm-quota-axi-lib.sh`    | Shared `quota-axi` compatibility floor for optional health checks                    |
| `fm-chrome-devtools-axi-lib.sh` | Shared `chrome-devtools-axi` compatibility floor and open-and-snapshot probe   |
| `fm-chrome-devtools-mcp.js` | Pinned chrome-devtools-mcp launcher for chrome-devtools-axi                       |
...
| `fm-anchor-lib.sh`       | Heartbeat ANCHOR block and session odometer for long-lived primary sessions |
...
| `fm-pr-review-chase.sh`  | Re-request a stale pending GitHub reviewer on a green tracked PR, then escalate one 24-hour wait |
