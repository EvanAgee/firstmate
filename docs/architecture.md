...
It also refuses to merge while the PR has any unresolved review thread, and fails closed when that thread query cannot be read; the script header owns that gate and its `--allow-unresolved-threads` override.
