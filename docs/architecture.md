...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, diagnosed stalled no-mistakes runs, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
A busy pane is otherwise exempt from staleness, but only until its latest `state/<id>.turn-ended` marker reaches `FM_BUSY_TURN_MAX_SECS`, or its `state/<id>.meta` spawn record reaches that age before any turn completes; past that bound it is routed through the same wedge escalation, with the identical reason, escalation count, and `demand-deep-inspection` marker.
When a stale escalation finds a Claude worker running a foreground Bash wait on a background-task output file, the watcher interrupts once and sends the output path, age, and last three lines if the file has not grown for `FM_BACKGROUND_OUTPUT_STALE_SECS`.
Its task marker prevents a second interrupt for the same output file and is removed at teardown.
...
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign only when `bin/fm-crew-state.sh` reports positive evidence that the crew is still working: a non-stalled active no-mistakes step attributed to that crew's current code, or an exact busy verdict from the semantic busy-state contract.
...
For an ordinary crew that has stopped, the normal-mode watcher first surfaces one stale wake, then applies that same cadence to an unchanged `paused:` or durable `captain-held` endpoint unless the backend reports its agent still alive.
A live agent whose last line is `paused:` and whose own semantic busy-state source reports its turn ended is parked by choice, so it joins the pause cadence without that stale wake; a Grok pane read only from rendered text never counts as parked.
An inconclusive liveness read (an unverified backend, an unreadable record) no longer forces the wedge ladder, and the secondmate idle-endpoint exemption is unchanged.
A `working` current-state verdict proves a run exists, not that it is moving, so it overrides a declared pause only while that run's own last activity is still fresh; a run that has gone quiet leaves the declared pause in force, which is what a worker stopped on a green PR looks like while its checks step keeps monitoring the open PR.
An idle pane is one signal and never escalates a wedge on its own: a worker escalates only when its second signal is quiet too, meaning no declared pause, no no-mistakes run whose last activity falls inside the stale window, and no armed PR merge watch behind an announced green PR.
The separate pipeline-activity freshness check is bounded and runs only at the generic wedge threshold, and a task with no delivery mode in its record drives no pipeline and skips it entirely.
The current-state read used for stalled-run detection happens at the stable-idle boundary before stale-hash suppression.
Its initial normal-mode status signal still surfaces through the no-verb path, while away mode self-handles that routine signal and owns the later recheck.
Fresh stale panes use the same current-state read before trusting the status log, so a non-stalled active run or a proven busy worker outranks an old captain-relevant status-log line left behind before validation.
No-change heartbeats stay absorbed, except the attended watcher presents one when `state/.last-anchor` is missing or older than `FM_ANCHOR_INTERVAL` so a live idle session still re-reads durable truth; `bin/fm-anchor-lib.sh` owns that ANCHOR and odometer contract.
...
It leads with a prominent bordered tangle banner, while `bin/fm-guard.sh` owns the in-session watcher-down banner and reminder policy so repeated guarded commands stay noisy without reprinting the full banner in the same episode.
`bin/fm-watcher-beat-alarm.sh` is the session-independent face for the same dead-watcher condition; [`wedge-alarm.md`](wedge-alarm.md) owns its install, fire, and optional re-arm contract.
...
It checks stalled current state at every stable-idle observation, while the daemon maintains distinct wedge and declared-pause recheck cadences.
...
The daemon escalates captain-relevant events, diagnosed stalled runs, and a bounded recheck for a declared pause that remains idle as one batched, single-line digest using the canonical `away-supervisor` kind from `bin/fm-operational-input.sh` so firstmate can distinguish it structurally from real messages.
...
Stalled escalation delivery writes `state/.subsuper-inject-wedged` and attempts a configured backend-independent active alert after `FM_MAX_DEFER_SECS` through `bin/fm-wedge-alarm-lib.sh` instead of silently deferring forever.
...
The same step removes a retired secondmate home's `.fm-secondmate-home` and `.fm-secondmate-parent` markers from a reused pool slot, because `treehouse return` keeps gitignored files and those markers would make the task's hooks treat the slot as that home.
It leaves the retired home's other private records in place.
...
`config/crew-dispatch.json` is inherited too, so secondmates use the same class pools when spawning their own crewmates.
...
`bin/fm-project-mode.sh` remains the one registry parser for fleet sync's `local-only` skip, home seeding's refusal and no-mistakes initialization, the spawn-time deviation notice, and registered PR merge policy checks.
...
PR-based task merges go through `bin/fm-pr-merge.sh`, which checks the task project's registered merge policy through `bin/fm-project-mode.sh` before recording `pr=` and any available `pr_head=` through `bin/fm-pr-check.sh` and calling `gh-axi pr merge`.
...
The [`bin/fm-pr-merge.sh`](../bin/fm-pr-merge.sh) header defines its per-project captain approval and review-thread checks.
...
A project whose own tooling reads `CLAUDE.md` or `AGENTS.md` as its instructions opts out with the marker line that script's header names, and the script then writes nothing there.
...
The captain-queue, blocked-task, rig-pool, captain-hold, and fleet-enrich reads are assembled by `bin/fm-api-reads.mjs`; `bin/fm-api-server.mjs` owns those HTTP routes and JSON contracts.
