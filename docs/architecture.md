...
The [`bin/fm-pr-merge.sh`](../bin/fm-pr-merge.sh) header defines the review-thread merge gate and its `--allow-unresolved-threads` override.
...
The parked-decision, blocked-task, rig-pool, captain-hold, and fleet-enrich reads are assembled by `bin/fm-api-reads.mjs`; `bin/fm-api-server.mjs` owns those HTTP routes and JSON contracts.
