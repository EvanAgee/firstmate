...
| omp | Run | `.pi/extensions/fm-primary-omp-turnend-guard.ts` maps `session_start`, `session_switch`, and `session_compact` onto the same wrapper sources and injects the marked digest with `pi.sendMessage`. | Use the canonical primary launcher documented in [harness configuration](configuration.md#harness-support); restored-session evidence distinguishes a fresh startup from resume or fork. |
