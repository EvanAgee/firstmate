...
A ship spawn may also record `issues=` when `--issue` is passed; `bin/fm-spawn.sh` owns that field and `bin/fm-issue-guard-lib.sh` owns the duplicate-claim preflight.
...
Claude's two Stop hooks own tokenless continuity, Cursor's stop hook parks on the watcher, Grok uses background-notify cycles, Codex uses bounded foreground checkpoints, Pi and pi-signed use the same two tracked Pi extensions, OMP uses its tracked native `.omp` extension, and OpenCode uses its TUI plugin.
...
OMP worker launches set `FM_OMP_HARNESS=omp` at the launch boundary, but that marker is inherited by everything the worker starts, so detection accepts it only when the innermost harness ancestor is itself an OMP process launched that way; a foreign agent started from an OMP worker's bash tool keeps its own identity.
The shared primary-scope predicate in [`turnend-guard.md`](turnend-guard.md#guard-predicates) requires an existing ordinary state directory, so only OMP's native primary adapter admits a first plain launch in a fresh checkout: it accepts an entirely absent canonical `<root>/state` path and creates it during startup, while state overrides and symlinked paths remain strict.
...
FM_CREW_STATE_BIN=bin/fm-crew-state.sh   # test override for the current-state reader used by watcher triage
...
FM_CLAUDE_AUTOARM_SYNC_WAIT_MS=800   # milliseconds the --claude turn-end guard waits for watcher health, a role-verified notifier claim, or a fresh epoch before deciding recovery ownership or failure progression
FM_CLAUDE_AUTOARM_EPOCH_FRESH=15   # seconds a recorded notifier outcome remains eligible for the current event epoch's recovery or failure decision
...
FM_CLAUDE_COORD_READY_TIMEOUT=30   # seconds the Claude coordinator waits for a successor arm to print started or attached
FM_CLAUDE_COORD_SUCCESSOR_TIMEOUT_STREAK=3   # consecutive successor-timeouts with no healthy watcher before the Claude coordinator stands down
FM_CLAUDE_NOTIFIER_COORD_WAIT=45   # seconds the parked Claude notifier waits for the coordinator to prove progress before a typed degraded failure
FM_CLAUDE_NOTIFIER_PARK_POLL=0.5   # seconds between parked Claude notifier polls
...
FM_BUSY_TURN_MAX_SECS=3600         # maximum age of a busy pane's latest state/<id>.turn-ended marker, or its state/<id>.meta spawn record before any turn completes, before the same wedge escalation used for a provably-working non-busy stale takes over
FM_BACKGROUND_OUTPUT_STALE_SECS=600  # age of the newest output in a Claude worker's current background-task session before a running foreground Bash wait on that file gets one interrupt and steer
...
FM_PIPELINE_ACTIVE_SECS=            # defaults to FM_STALE_ESCALATE_SECS; a no-mistakes step whose active_steps last_activity is newer than this counts as a live pipeline, so an idle pane alone never escalates a wedge
FM_PIPELINE_PARKED_MAX=1200         # active_steps last_activity age in seconds at which a running/fixing step is reported stalled; awaiting_agent without a live PID is stalled immediately, while approval gates stay parked; notification only, no automatic recovery
FM_PIPELINE_CHECK_TIMEOUT=          # defaults to FM_CHECK_TIMEOUT (30); seconds allowed for the bounded `no-mistakes axi status` read the wedge gate makes at the escalation threshold
