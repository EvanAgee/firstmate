...
Claude's Stop `asyncRewake` hook owns tokenless re-arm cycles, Cursor's stop hook parks on the watcher, Grok uses background-notify cycles, Codex uses bounded foreground checkpoints, Pi and pi-signed use the same two tracked primary extensions, omp uses its two explicitly loaded extensions, and OpenCode uses its TUI plugin.
`bin/fm-omp.sh` is the canonical primary omp launch command; its header owns the explicit extension argv.
