...
FM_PIPELINE_PARKED_MAX=1200         # seconds an active_steps agent may go quiet, or an awaiting_agent gap may sit with no live agent PID, before bin/fm-crew-state.sh reclassifies a running/fixing run-step `stalled` instead of `working`, so a died agent (a model usage limit, a killed process, a lost socket) surfaces instead of reading as healthy work
