...
Spawn exports `CHROME_DEVTOOLS_AXI_MCP_PATH` to that launcher and `CHROME_DEVTOOLS_AXI_SESSION` to the task id so workers do not share the default bridge or pick up `@latest`.
