...
family_for_basename() {
  case "$1" in
    ...
    fm-send-secondmate-marker.test.sh|fm-shared-captain-inheritance.test.sh)
      printf '%s\n' secondmate
      ;;
    fm-api.test.sh|fm-api-reads.test.sh|fm-bootstrap.test.sh|fm-chrome-devtools-axi.test.sh|fm-fleet-sync.test.sh|fm-gate-refuse.test.sh|fm-gotmp.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-gate-refuse*|bin/fm-lock*|bin/fm-quota-axi-lib.sh|bin/fm-api.sh|bin/fm-api-server.mjs|bin/fm-api-reads.mjs)
      printf '%s\n' session-bootstrap
      ;;
