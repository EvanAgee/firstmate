#!/usr/bin/env bash
# Verify the selected OMP executable has Firstmate's required lifecycle surface.
# Usage: fm-omp-capabilities.sh [--print-binary]
# Success is silent unless --print-binary requests the resolved executable path.
...
require_help_token '--config=' '--config=<value>'
require_help_token '--cwd=' '--cwd=<value>'
