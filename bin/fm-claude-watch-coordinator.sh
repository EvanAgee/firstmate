...
# The liveness beacon is touched once per watcher cycle, so a healthy cycle's
# beacon can legitimately age up to FM_POLL seconds between touches.
# fm_poll_derived_grace (bin/fm-wake-lib.sh) is the single owner of that
# max(300, poll+60) derivation (docs/turnend-guard.md).
