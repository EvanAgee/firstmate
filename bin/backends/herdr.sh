...
#
# Requires: herdr (CLI + socket), jq (JSON parsing). Bootstrap detects herdr
# through fm_backend_required_tools only when herdr is the resolved backend;
# it detects jq as part of the universal toolchain in bin/fm-bootstrap.sh.
# This adapter also gates both tools again before spawning.
...
# human could have renamed or repurposed it in the interim), and close it only
# when the pane's agent state is positively confirmed no-agent by
# fm_backend_herdr_pane_agent_state. A working, idle, done, blocked, or
# unreadable state all preserve the tab, because idle, done, and blocked agents
# are still live sessions - only a confirmed agent_not_found (or a proven husk
# shell) licenses the close. This is belt-and-suspenders against any other
...
#   live     - `agent get` reports `working`, OR reports `idle`/`done`/
#              `blocked` while the pane's foreground is NOT a lone bare shell:
#              a genuine still-registered agent (at rest), never a
#              close-and-replace candidate. An idle or blocked agent still
#              owns the foreground process, so it reads live.
#   no-agent - `agent get` reports `idle`/`done`/`blocked` BUT the pane
#              provably holds a lone bare shell
#              (fm_backend_herdr_pane_idle_shell_sample succeeds): no agent
#              process is running, so the registered binding is stale. This is
#              the provider-death husk - a worker's model provider died mid-
#              task, the omp TUI exited and left a bare shell, and herdr kept
#              the dead binding - and it IS relaunchable and closeable.
#              `working` never reaches this check: an actively generating
#              agent is foreground by definition, so a working binding is
#              trusted as live without the shell recheck.
...
fm_backend_herdr_pane_agent_state() {  # <session> <pane_id>
  ...
  case "$status" in
    working) printf 'live' ;;
    idle|done|blocked)
      # A registered non-working status normally means a genuine agent at
      # rest. But a provider-death husk - omp exited, bare shell left behind,
      # herdr still reporting the dead binding's last status - reads the same
      # idle/done/blocked value. Reusing the single idle-shell proof owner:
      # if the pane's foreground provably IS a lone bare shell, no agent
      # process is running and the binding is stale, so this is the no-agent
      # husk a relaunch must replace. The single-shot sample is enough (a
      # husk is a stable bare shell); a transient prompt helper that makes
      # the sample fail leaves the verdict live - fail-safe toward refusing
      # a relaunch rather than risking a live agent.
