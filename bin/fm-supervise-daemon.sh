...
# The one owner of the active-alert channel machinery (channel directives,
# bounded dispatch, the FM_WEDGE_ALARM_EXEC test seam) shared with the
# watcher-beat launcher (bin/fm-watcher-beat-alarm.sh). The daemon keeps its own
# wedge marker and rate-limit semantics; only the how-an-alert-fires layer is
# shared.
...
# WEDGE_ALARM_TIMEOUT_SECS_DEFAULT and WEDGE_ALARM_NOTIFIER_PID live in
# bin/fm-wedge-alarm-lib.sh, the alert-channel owner; WEDGE_ALARM_LAST_EPOCH
# stays here with the daemon's rate-limit semantics.
...
# The active-alert channel machinery lives in bin/fm-wedge-alarm-lib.sh (the
# one how-an-alert-fires owner, shared with bin/fm-watcher-beat-alarm.sh).
# The daemon-level contract stays here: inject_wedge_alarm below owns the
# wedge marker, the tmux flash, and the rate-limit semantics; every active
# alert routes through wedge_alarm_notify and the FM_WEDGE_ALARM_EXEC seam.
...
handle_wake() {  # <reason> <state>
  ...
  local kind="" arg="" crew_line="" observed_generation="" stalled_rc
    ...
    stale:*)  kind=stale; arg="${reason#stale: }"; stale_detail="${arg#"$arg"}"
              ...
              case "$queue_key" in
                "$arg|pipeline-stall|"*)
                  ...
                  if escalate_stalled "$arg" "$state" "$stale_detail" "$episode" ""; then
