...
remote_secondmate_teardown() {
  ...
  rm -f -- "$STATE/$ID.meta" "$STATE/$ID.turn-ended" "$STATE/$ID.turn-continue" "$STATE/$ID.pane-tail" \
    "$STATE/$ID.stalled-output-"* "$STATE/$ID.control-exit"
...
validate_pr_poll_cleanup() {
  ...
  for artifact in "$state_dir/$id.check.sh" "$state_dir/$id.pr-poll" \
    ...
    "$state_dir/$id.pr-review-chase" "$state_dir/$id.check-trust"; do
...
remove_pr_poll_artifacts() {
  ...
  rm -f "$state_dir/$id.check.sh" "$state_dir/$id.pr-poll" \
    "$state_dir/$id.pr-poll-registration" "$state_dir/$id.pr-poll-retirement" \
    "$state_dir/$id.pr-review-chase" "$state_dir/$id.check-trust" || return 1
...
reap_task_backend_process_group() {  # <label>
  ...
  # fm_tmux_display_message, never a raw display-message: for a gone window a
  # raw read answers for the client's current window and returns a LIVE
  # unrelated pane's pid, which this function would then kill as a process
  ...
  case "$leader" in ''|*[!0-9]*)
    echo "warning: lsof is unavailable; cannot resolve the tmux pane leader for $ID" >&2
    return 0
    ;;
  esac
...
cleanup_firstmate_home_children() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    rm -f "$sub_state/$child_id.turn-ended" \
      "$sub_state/$child_id.meta" "$sub_state/$child_id.pane-tail" \
      "$sub_state/$child_id.stalled-output-"* "$sub_state/$child_id.control-exit" \
