...
# Worker skill launch paths were checked against real local help on 2026-09-04.
# Claude Code 2.1.260 advertises --append-system-prompt[-file], Pi 0.84.3
# advertises repeatable --skill <path>, and OMP 18.1.6 says
# --append-system-prompt reads file contents. Claude and OMP receive one
# task-temp concatenation whose first line records caveman: full and ponytail:
# full; Pi and pi-signed receive each available skill directory separately.
...
# one-line brief prefix, as do Cursor and Muse, whose executables were absent
# from this machine. pi-signed was also absent; its existing verified adapter
# contract shares Pi's CLI. Secondmate launches receive none of these additions.
...
launch_template() {
  ...
  case "$harness" in
    ...
    pi|pi-signed)
      ...
      ;;
    omp)
      # omp (Oh My Pi): native project-discovered extension-owned supervision.
      # The explicit -e path names the exact same tracked file native discovery
      # sees; OMP path-resolves and deduplicates before loading, so this
      # guarantees the integration without registering it twice. --auto-approve
...
case "$HARNESS" in
  pi|pi-signed)
    ...
    ;;
  omp)
    # OMP identity, Bun, and session-dir resolution is owned by the native OMP
    # block above (fm-omp-capabilities.sh / fm-omp-process-lib.sh); it sets
    # OMP_BIN and the canonical identities. Prefix the launch with the exact
    # OMP harness marker so fm-harness.sh identifies the worker as omp.
...
if [ "$KIND" != secondmate ]; then
  ...
  if [ -n "$WORKER_SKILL_LEVELS" ]; then
    case "$HARNESS" in
      claude|omp)
        WORKER_SKILL_PROMPT="$TASK_TMP/worker-skills.md"
        {
      ...
      codex|grok|kimi|cursor|muse|opencode)
        BRIEF_DELIVERY="$TASK_TMP/brief.md"
...
if [ "$KIND" != secondmate ]; then
  ...
  case "$HARNESS" in
    claude*)
      ...
      # Captain standing rule (2026-08-20): a Fable-model crew worker must never
      # spawn subagents (each is an independent Claude context and multiplies
...
{"hooks":{"UserPromptSubmit":[{"hooks":[{"type":"command","command":"$j_submit"}]}],"Stop":[{"hooks":[{"type":"command","command":"$j_stop"}]}],"StopFailure":[{"hooks":[{"type":"command","command":"$j_stopfail"}]}],"SessionEnd":[{"hooks":[{"type":"command","command":"$j_sessionend"}]}]}$claude_perms}
EOF
