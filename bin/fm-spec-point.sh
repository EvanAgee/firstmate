...
# own first, then land.
#
# It reads the task's recorded worktree from state/<task>.meta, lists the
# docs/specs/*.md files that worktree's checked-out commit added or changed
# against the project's default branch, and requires exactly one. It lints that
# file where the hook will read it, in the worktree, with $FM_SPEC_LINT (default
# ~/.agents/skills/spec-lint/spec-lint), then rewrites the brief's first "Spec:"
# line to "Spec: <path relative to the worktree>", which is the line the hook
# reads and the way it resolves a relative path at landing time. Every other
# byte of the brief is kept.
#
# It refuses, exits 1, and leaves the brief untouched when the lane changed no
# spec or more than one, when the spec does not lint, or when the brief has no
# "Spec:" line. A brief that already names another spec is left alone (exit 0),
# so running this before every landing is safe. It never checks the proof's
# acceptance criterion ids; the hook owns that check at landing.
#
...
specs=$(git -C "$WORKTREE" diff --name-only --no-renames --diff-filter=d "$BASE...HEAD" -- 'docs/specs/*.md')
count=$(printf '%s' "$specs" | grep -c '' || true)
if [ "$count" -ne 1 ]; then
  die "refused: $ID's lane in $WORKTREE changed $count specs under docs/specs/ against $BASE${specs:+: $(printf '%s' "$specs" | tr '\n' ' ')}; it must change exactly one"
fi
SPEC=$specs

[ -x "$SPEC_LINT" ] || die "error: spec linter not found at $SPEC_LINT; set FM_SPEC_LINT to its path"
if ! lint_out=$("$SPEC_LINT" "$WORKTREE/$SPEC" 2>&1); then
  printf '%s\n' "$lint_out" >&2
  die "refused: $SPEC in $ID's lane does not lint clean; the worker fixes it before landing"
fi
