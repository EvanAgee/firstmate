...
[`.github/workflows/unslop.yml`](.github/workflows/unslop.yml) is a separate PR-only gate that scans the supported JavaScript and TypeScript files a pull request changes and fails the build only on error-tier gateable AI-slop findings; that workflow owns its pinned analyzer versions and gating rules.
