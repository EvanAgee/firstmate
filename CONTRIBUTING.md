...
It runs for every PR opening and body edit, then reads the current body with a short retry window for multi-write updates and transient API failures.
The check fails if the final read fails or the signature is still missing when retries end.
...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `CONTEXT.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, `skills/`, and `skills-lock.json`.
  Skill placement, the installer-hide flag, and the vendored lock-file rule are owned by the README's "Two-tier skill layout".
...
Tracked changes to firstmate itself - `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `CONTEXT.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, `skills/`, and `skills-lock.json` - ship through the `no-mistakes` pipeline on a feature branch and require an explicit merge approval.
...
[`.github/workflows/unslop.yml`](.github/workflows/unslop.yml) is a separate PR-only gate that scans the supported JavaScript and TypeScript files a pull request changes, scans the same files at the pull request's base commit, and fails the build only on error-tier gateable AI-slop findings the pull request newly introduces; pre-existing findings stay report-only even in touched files, an unslop execution failure or unreadable report is announced loudly in the job log but does not block the pull request, and that workflow owns its pinned analyzer versions and gating rules.
...
Skip only that one assertion, keep the rest of the script assertive, and never let a skip absorb a genuine regression on a clean machine.
