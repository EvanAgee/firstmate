...
[`.github/workflows/unslop.yml`](.github/workflows/unslop.yml) is a separate PR-only gate that scans the supported JavaScript and TypeScript files a pull request changes and fails the build on error-tier gateable AI-slop findings or analyzer execution failures; that workflow owns its pinned analyzer versions and gating rules.
